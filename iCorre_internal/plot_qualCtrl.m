...
for ii = 1:numel(matFile)  % different iterations
...
%% compute and plot metrics
    ...
    if strcmp(data.options_label, 'RMC')  % rigid motion correction
        ...
        print(gcf, '-r0', fullfile(path_QC , ['RMC_metrics','-iter',num2str(ii)]), '-dpng'); %png format
    ...
    elseif strcmp(data.options_label, 'NRMC')
        ...
        print(gcf, '-r0', fullfile(path_QC , 'NRMC_metrics','-iter',num2str(ii)), '-dpng'); %png format
    ...
    figure;
        ...
        print(gcf, '-r0', fullfile(path_QC , ['RMC_shifts','-iter',num2str(ii)]), '-dpng'); %png format
    elseif strcmp(data.options_label, 'NRMC')
    ...
    figure;
        ...
        ax3 = subplot(313); plot(shifts_y);  title('displacements along y','fontsize',14,'fontweight','bold')
            xlabel('timestep','fontsize',14,'fontweight','bold')
        linkaxes([ax1,ax2,ax3],'x')
        print(gcf, '-r0', fullfile(path_QC , 'NRMC_shifts','-iter',num2str(ii)), '-dpng'); %png format
    end
end
