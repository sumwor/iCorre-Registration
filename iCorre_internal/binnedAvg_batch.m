...
for j = 1:numel(stack_path)
    ...
    idx.DS_local = size(curr.stack, 3);
    if j<numel(stack_path) %Load next stack to pick up remainder of frames for averaging
        ...
        curr.stack = cat(3,curr.stack,next.stack(:,:,1:bin_width:end)); %Concatenate with first n (=downsample_factor) frames of next_stack
    end
    
    for k = 1:idx.DS_local
        if mod(kk,1000) == 1
            kk
        end
        temp_idx = [k : k + bin_width - 1]; %Segment to average
...
saveTiff(stack_downsample,stackInfo.tags,fullfile(...
    save_dir,[filename(5:28) '_DS' num2str(bin_width) '.tif'])); %Save max projection for entire session
