...
%180724mjs Commented out most of the console text output to simplify logging.

% can be optimized by gpu computing: meshgrid; fftn;
...
%     fprintf('Registering the first %i frames just to obtain a good template....',init_batch); %Commented to simplify console output 180724mjs
    template_in = median(Y_temp,nd+1)+add_value;
    fftTemp = fftn(template_in);
    for t = 1:size(Y_temp,nd+1)        
        if nd == 2
            [~,Greg] = dftregistration_min_max_gpu(fftTemp,fftn(Y_temp(:,:,t)),us_fac,-max_shift,max_shift,options.phase_flag);
        end
        if nd == 3 
...
for i = 1:length(xx_us)
    for j = 1:length(yy_us)
        for k = 1:length(zz_us)
            ...
            [Nc{i,j,k},Nr{i,j,k},Np{i,j,k}] = meshgrid(nc,nr,np); % meshgrid can use gpu
...
%                Ytm = zeros(sizY(1),sizY(2),min(t+bin_width-1,T)-t+1,'single');
                Ytm = single(read_file(YGPU, t, min(t+bin_width-1,T)-t+1, [], tiffInfo));
...
%                 end
            ...
            case 'mem'
                if nd == 2; Ytm = single(YGPU.(var_name)(:,:,t:min(t+bin_width-1,T))); end
                if nd == 3; Ytm = single(YGPU.(var_name)(:,:,:,t:min(t+bin_width-1,T))); end
            case 'mat'
                if nd == 2; Ytm = single(YGPU(:,:,t:min(t+bin_width-1,T))); end
                if nd == 3; Ytm = single(YGPU(:,:,:,t:min(t+bin_width-1,T))); end
            case 'raw'
                Ytm = single(read_raw_file(YGPU,t,min(t+bin_width-1,T)-t+1,FOV,bitsize));                
        end
        ...
        for ii = 1:lY
            ...
            shifts_temp = gpuArray(zeros(length(xx_s),length(yy_s),length(zz_s),nd)); 
            diff_temp = gpuArray(zeros(length(xx_s),length(yy_s),length(zz_s)));
            if numel(M_fin) > 1      
                if use_windowing
                    if nd == 2; out_rig = dftregistration_min_max_gpu(fftTempMat,fftn(han(Yt)),us_fac,-max_shift,max_shift,phase_flag); lb = out_rig(3:4); ub = out_rig(3:4); end
                    ...
                    if nd == 2; out_rig = dftregistration_min_max_gpu(fftTempMat,fftn(Yt),us_fac,-max_shift,max_shift,phase_flag); lb = out_rig(3:4); ub = out_rig(3:4); end
                ...
                for j = 1:length(yy_s)           
                    for k = 1:length(zz_s)
                        if nd == 2
                            ...
                            output = dftregistration_min_max_gpu(fftTemp{i,j,k},fftY{i,j,k},us_fac,lb-max_dev(1:2),ub+max_dev(1:2),phase_flag);  
                        elseif nd == 3
            ...
            switch lower(options.shifts_method)
                case 'fft'
                    ...
                    if flag_interp    
                        Mf{ii} = cell2mat_ov_sum_gpu(M_fin,xx_us,xx_uf,yy_us,yy_uf,zz_us,zz_uf,overlap_post,sizY,Bs) - add_value;
                    else            
                        Mf{ii} = cell2mat_ov_gpu(M_fin,xx_us,xx_uf,yy_us,yy_uf,zz_us,zz_uf,overlap_post,sizY) - add_value;
                    end                    
                ...
                otherwise
                    ...
                    if nd == 3                
                        ...
                        shifts_up(2:2:end,:,2) = shifts_up(2:2:end,:,2) + col_shift;
                        Yt = gather(Yt); shifts_up = gather(shifts_up);
                        Mf{ii} = imwarp(Yt,-cat(3,shifts_up(:,:,2),shifts_up(:,:,1)),options.shifts_method,'FillValues',fill_value);  
                    end   
            end
           
            Mf{ii}(Mf{ii}<minY)=gather(minY);
            Mf{ii}(Mf{ii}>maxY)=gather(maxY);
        end
        ...
        Mf = cell2mat(Mf); % what if not converting
...
%         prevstr=str; %Commented to simplify console output 180724mjs
        % update template
        if upd_template
            cnt_buf = cnt_buf + 1;
            ...
            elseif strcmpi(method{1},'median');
                if cnt_buf <= buffer_width
                    if nd == 2; buffer_med(:,:,cnt_buf) = cell2mat_ov_gpu(new_temp,xx_s,xx_f,yy_s,yy_f,zz_s,zz_f,overlap_pre,sizY); end
                    if nd == 3; buffer_med(:,:,:,cnt_buf) = cell2mat_ov_gpu(new_temp,xx_s,xx_f,yy_s,yy_f,zz_s,zz_f,overlap_pre,sizY); end
                else
                    buffer_med = circshift(buffer_med,[zeros(1,nd),-1]);
                    if nd == 2; buffer_med(:,:,buffer_width) = cell2mat_ov_gpu(new_temp,xx_s,xx_f,yy_s,yy_f,zz_s,zz_f,overlap_pre,sizY); end
                    if nd == 3; buffer_med(:,:,:,buffer_width) = cell2mat_ov_gpu(new_temp,xx_s,xx_f,yy_s,yy_f,zz_s,zz_f,overlap_pre,sizY); end
                end
                template = mat2cell_ov(nanmedian(buffer_med,nd+1),xx_s,xx_f,yy_s,yy_f,zz_s,zz_f,overlap_pre,sizY);
            end
            temp_mat = cell2mat_ov_gpu(template,xx_s,xx_f,yy_s,yy_f,zz_s,zz_f,overlap_pre,sizY);
            if use_windowing
                fftTemp = cellfun(@fftn, cellfun(@han,template, 'un',0),'un',0);            
                fftTempMat = fftn(han(temp_mat));            
            else
...
if it == iter
    ...
    template = cell2mat_ov_gpu(template,xx_s,xx_f,yy_s,yy_f,zz_s,zz_f,overlap_pre,sizY);
end
if memmap;
